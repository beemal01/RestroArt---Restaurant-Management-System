...
function openMenuPop(card) {
    mpFoodId = card.getAttribute('data-id');
...
// Card click open popup - use event delegation since cards are loaded dynamically by loadFoods()
...
document.addEventListener('click', function(e) {
    var btn = e.target.closest('.madd');
    if (btn) {
        e.stopPropagation();
        openMenuPop(btn.closest('.mcard'));
    }
});

// Heart toggle (no popup)
document.addEventListener('click', function(e) {
    var btn = e.target.closest('.mhrt');
    if (btn) {
        e.stopPropagation();
